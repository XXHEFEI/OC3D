...
def get_status_dict():
    ...
    try:
        r = subprocess.run(
            ...
            capture_output=True, text=True, timeout=30, encoding="utf-8",
            env={**os.environ, "BAMBU_MODE": os.environ.get("BAMBU_MODE", "local")})
        return {"raw": r.stdout.strip() if r.stdout else ""}
...
def notify(title, message, snapshot=None):
    ...
    script = os.path.join(os.path.dirname(__file__), "bambu.py")
    try:
        cmd = ["py", script, "notify", "--title", title, "--message", message]
        if snapshot:
            cmd.extend(["--image", snapshot])
        subprocess.run(cmd, capture_output=True, timeout=15, encoding="utf-8")
    except Exception:
        pass
...
def pause_print():
    ...
    script = os.path.join(os.path.dirname(__file__), "bambu.py")
    try:
        r = subprocess.run(["py", script, "pause"],
                          capture_output=True, text=True, timeout=30, encoding="utf-8")
