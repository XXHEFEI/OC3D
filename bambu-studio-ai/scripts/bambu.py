...
def _ensure_x509():
    ...
    try:
        ...
        BAMBU_APP_CERT_ID = _cert_obj.subject.get_attributes_for_oid(_x509.oid.NameOID.COMMON_NAME)[0].value
    except Exception:
        BAMBU_APP_CERT_ID = None
    return True
...
def sign_message_x509(message_dict):
    ...
    private_key = serialization.load_pem_private_key(
        BAMBU_APP_PRIVATE_KEY.encode(), password=None, backend=default_backend()
    )
...
def ftp_upload_via_curl(ip, access_code, local_path, remote_filename):
    ...
    try:
        ...
        result = subprocess.run(
            ["curl", "--ftp-ssl-reqd", "-k", "--netrc-file", netrc_path, "-T", local_path, ftps_url],
            capture_output=True,
            timeout=60,
            check=True,
        )
...
class CloudBackend:
    def __init__(self):
        ...
        if not email or not password:
            ...
            if not email:
                print("   export BAMBU_EMAIL='your@email.com'")
            if not password:
                print("   export BAMBU_PASSWORD='your_password'")
            sys.exit(1)
    ...
    def pause(self):
        self.client._request("POST", f"/v1/devices/{self.device_id}/commands", json={"print": {"command": "pause"}})

    def resume(self):
        self.client._request("POST", f"/v1/devices/{self.device_id}/commands", json={"print": {"command": "resume"}})

    def stop(self):
        self.client._request("POST", f"/v1/devices/{self.device_id}/commands", json={"print": {"command": "stop"}})

    def set_light(self, on):
        mode = "on" if on else "off"
        self.client._request("POST", f"/v1/devices/{self.device_id}/commands", json={"system": {"led_mode": mode}})
...
class LocalBackend:
    def __init__(self):
        ...
        if not all([ip, serial, access_code]):
            ...
            if not serial:
                print("   export BAMBU_SERIAL='01P00Axxxxxxx'")
            if not access_code:
        ...
        # Try ssl_verify=False first (newer bambulabs-api), fall back to no arg
        try:
    ...
    @staticmethod
    def _read_3mf_metadata(filepath, plate_number=1):
        """Read print parameters from a .3mf file's Metadata directory.

        Returns a dict with keys matching the MQTT project_file payload
        so the command matches what Bambu Studio configured at slice time.
        Fields not present in the .3mf fall back to sensible defaults.

        Note: bed_leveling / flow_cali / vibration_cali / layer_inspect are
        *print-dialog* choices set in Bambu Studio at send time — they are
        NOT embedded in the .3mf and remain as hardcoded defaults here.
        """
        import zipfile, json as _json, xml.etree.ElementTree as _ET

        meta = {}
        try:
            with zipfile.ZipFile(filepath, "r") as zf:
                # ── plate_{n}.json — bed_type, nozzle_diameter ──
                plate_json_name = f"Metadata/plate_{plate_number}.json"
                if plate_json_name in zf.namelist():
                    plate = _json.loads(zf.read(plate_json_name).decode("utf-8"))
                    meta["bed_type"] = plate.get("bed_type", "textured_plate")
                    meta["nozzle_diameter"] = plate.get("nozzle_diameter", 0.4)
                    meta["filament_ids"] = plate.get("filament_ids", [0])
                    meta["filament_colors"] = plate.get("filament_colors", [])
                    meta["first_extruder"] = plate.get("first_extruder", 0)

                # ── plate_{n}.gcode.md5 ──
                md5_name = f"Metadata/plate_{plate_number}.gcode.md5"
                if md5_name in zf.namelist():
                    meta["gcode_md5"] = zf.read(md5_name).decode("utf-8").strip()

                # ── slice_info.config (XML) — timelapse, printer_model ──
                if "Metadata/slice_info.config" in zf.namelist():
                    root = _ET.fromstring(
                        zf.read("Metadata/slice_info.config").decode("utf-8")
                    )
                    for elem in root.iter("metadata"):
                        key = elem.attrib.get("key", "")
                        val = elem.attrib.get("value", "")
                        if key == "timelapse_type":
                            meta["timelapse"] = val != "0"
                        elif key == "printer_model_id":
                            meta["printer_model_id"] = val
                        elif key == "support_used":
                            meta["support_used"] = val == "true"
                ...
                if "Metadata/project_settings.config" in zf.namelist():
                    ...
                    if "curr_bed_type" in psc:
                        meta["curr_bed_type_label"] = psc["curr_bed_type"]

        except Exception as e:
            print(f"⚠️ reading .3mf metadata failed ({e}), using defaults")

        return meta

    def start_print(self, filename, plate_number=1, ams_mapping=None, ams_mapping2=None):
        """
        Start printing. Supports both .gcode and .3mf files.

        For .3mf files: uploads .3mf via FTP, sends project_file MQTT
          command with payload populated from .3mf metadata (bed_type,
          timelapse, nozzle_diameter, etc.) so parameters always match
          what was configured in Bambu Studio at slice time.
        For .gcode files: uses the standard start_print API.
        ...
        if filename.lower().endswith(".3mf") and _fos.path.exists(filename):
            # Read metadata embedded by Bambu Studio at slice time so
            ...
            # Unique remote name to avoid "553 Could not create file" on overwrite
            import time as _time
            base = _fos.path.splitext(_fos.path.basename(filename))[0]
            remote_filename = f"{base}_{int(_time.time())}.3mf"

            print(f"📤 Uploading {filename} to printer via FTP...")
            try:
                # Step 1: Upload .3mf to printer (use module-level ftp_upload_file
                # which has proper TLS session reuse, bypassing library's decorator)
                ftp_upload_file(self.ip, self.access_code, filename, remote_filename)
                print(f"✅ Upload complete: {remote_filename}")

                # Step 2: Send project_file MQTT command
                ...
                payload = {
                    "print": {
                        ...
                        "bed_leveling": slicing_meta.get("bed_leveling", True),
                        "auto_bed_leveling": 1,
                        "flow_cali": slicing_meta.get("flow_cali", False),
                        "vibration_cali": slicing_meta.get("vibration_cali", True),
                        "layer_inspect": slicing_meta.get("layer_inspect", True),
                        "ams_mapping": mapping,
                        "ams_mapping2": ams_mapping2 if ams_mapping2 else "",
                        "use_ams": use_ams,
                        "cfg": "0",
                        "extrude_cali_flag": 2,
                ...
                mqtt = self.printer.mqtt_client
                mqtt._client.publish(mqtt.command_topic, _json.dumps(payload))
...
def cmd_light(state):
    b = get_backend()
    try:
        b.set_light(state == "on")
        print(f"💡 Light {'ON' if state == 'on' else 'OFF'}")
    finally:
        b.disconnect()
...
def cmd_speed(mode):
    ...
    try:
        b.set_speed(level)
        print(f"🏎️ Speed: {mode.capitalize()}")
    finally:
        b.disconnect()


def cmd_print(filename, confirmed=False, ams_mapping=None, ams_mapping2=None):
    ...
    if isinstance(ams_mapping2, str) and ams_mapping2:
        ams_mapping2 = [int(x.strip()) for x in ams_mapping2.split(",")]
    if not confirmed:
    ...
    b = get_backend()
    try:
        b.start_print(filename, plate_number=1, ams_mapping=ams_mapping, ams_mapping2=ams_mapping2)
    ...
    try:
        ...
        result = subprocess.run(
            [
                ...
                "tcp",
                "-i",
                f"rtsps://bblp:{ac}@{ip}:322/streaming/live/1",
                "-frames:v",
                "1",
                out,
            ],
...
def main():
    ...
    sp_info.add_argument("--json", action="store_true", help="JSON output")
    ...
    p.add_argument("filename")
    p = sub.add_parser("gcode", help="Send raw G-code (local only)")
    p.add_argument("code")
    p = sub.add_parser("notify", help="Send notification")
    p.add_argument("--title", default="Bambu Studio AI")
    p.add_argument("--message", required=True)
    p.add_argument("--image")
    p = sub.add_parser("light")
    p.add_argument("state", choices=["on", "off"])
    p = sub.add_parser("speed")
    p.add_argument("mode", choices=["silent", "standard", "sport", "ludicrous"])
    ...
    if args.command == "status":
        ...
        cmd_print(args.filename, confirmed=args.confirmed, ams_mapping=getattr(args, "ams_mapping", None), ams_mapping2=getattr(args, "ams_mapping2", None))
