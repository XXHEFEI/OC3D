...
- **魔法衣橱（本半负责）**：6×6 DIY 换装 → 展示三视图 → 二维码下载该搭配的 OSS 打包资源（**不打印**）。

当前前端是**纯视觉演示**：不调 OpenClaw 或 Meshy；旧的升维后端保留作未来恢复真实生成时的备用能力。
...
| `combos.json` | 搭配注册表，**6×6**（衣/配均为 0-5，0=经典原皮/卸下配饰）。合成图 `static/xixi_diy/{cloth}_cloth_o{ornament}.png`。 |
| `static/prompt.generate.js` | 旧的真实升维提示词，当前纯视觉前端不调用；恢复真实生成时再启用。 |
| `static/gateway.js` | `GatewayChat` 类；连接时先 `fetch('/api/gateway-token')` 取 token。 |
| `templates/customize.html` | 换装页（6×6），选择搭配后直接跳转三视图预览。 |
| `templates/preview_diy.html` | 三视图预览页，使用固定演示进度；确认后携带衣/配编号跳转 takeaway。 |
| `templates/takeaway.html` | 扫码页；按搭配编号加载本地预制二维码 PNG。 |
| `static/qr/` | 36 张预制二维码、`manifest.json`（搭配编号→固定 OSS 下载 URL）与生成说明。 |
| `templates/select.html` | 双路枢纽（左=打印，右=换装）。 |
| `task_state.py` | 状态持久化 `work/{task_id}_state.json`。 |

**当前端到端流程**：customize 选择衣/配 → preview_diy 展示 `static/xixi_diy/3D/` 三视图与演示进度 → takeaway 以 `c`/`o` 加载 `static/qr/{c}_{o}.png` → 扫码下载 `static/qr/manifest.json` 映射的 OSS ZIP。
...
- 纯视觉链路已切换为：换装（6×6）→ 三视图预览 → OSS 下载二维码预留。
- 36 组搭配的主图、正/侧/背视图均已放入 `static/xixi_diy/`。
- Meshy 真路单图曾验证过（`--image` → 真 STL，已缩到 10cm），但当前前端不调用。
...
1. **前端现场验收（当前重点）**：逐一确认 36 组搭配的主图、三视图和二维码编号匹配。
2. **OSS 资源维护**：OSS ZIP 如有改名/迁移，只更新 `static/qr/manifest.json` 并重跑 `scripts/generate_diy_qr.py`。
4. **真实升维恢复（非当前工作）**：若再次启用 Meshy/一体机，再恢复 `customize → /api/generate → OpenClaw` 的接线并重新验证额度与下载包。

## 6. 在等什么

- **OSS 下载域名与访问策略**：当前使用固定公共下载 URL；二维码内容由 `static/qr/manifest.json` 集中维护。
- **36 个压缩包的交付清单**：确认每包内容、OSS 文件名和 `manifest.json` 映射一致。
- **现场网络验收**：手机扫码后需要能访问下载域名；若现场无公网，需另行准备局域网下载服务。

一体机与 Meshy 的历史验证记录保留在下一节，当前不属于前端演示链路。
